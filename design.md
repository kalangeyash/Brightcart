...
typography:
  hero-display:
    fontFamily: "SF Pro Display, system-ui, -brightcart-system, sans-serif"
    fontSize: 56px
    fontWeight: 600
    lineHeight: 1.07
  ...
  display-lg:
    ...
    letterSpacing: 0
  display-md:
    fontFamily: "SF Pro Text, system-ui, -brightcart-system, sans-serif"
    fontSize: 34px
    fontWeight: 600
    lineHeight: 1.47
...
components:
  ...
  text-link-on-dark:
    ...
    typography: "{typography.body}"
  text-link-secondary-on-dark:
    backgroundColor: transparent
    textColor: "{colors.secondary-on-dark}"
    typography: "{typography.body}"
  global-nav:
...
Brightcart's web presence is a masterclass in **reverent product photography framed by near-invisible UI**. Every page is a stack of edge-to-edge product "tiles" — alternating light and dark canvases, each centered on a hero headline, a one-line tagline, two tiny pill CTAs in the brand's amber-and-pine pair, and an impossibly crisp product render. Nothing competes with the product. Typography is confident but quiet; color is either pure white, an off-white parchment, or a near-black tile; interactive elements are carried by exactly two accents — a warm primary and a cool secondary — and nothing else.

Density is unusually low even by contemporary SaaS standards. Each tile occupies roughly one viewport, and there is no decorative chrome — no borders, no gradients, no decorative frames, no shadows on headlines. Elevation appears only when a product image rests on a surface (a single soft `rgba(0, 0, 0, 0.22) 3px 5px 30px` drop for visual weight). The result is a catalog that feels more like a museum gallery: the wall disappears and the artifact takes over.

Store and shop surfaces retain the same chassis but switch modes. The product configurator (iPhone 17 Pro, accessories grid) introduces a tight grid of white utility cards at `{rounded.lg}` (18px) radius with a thin border, paired with a persistent thin sub-nav strip. The environment page leans darker and more editorial. Across all five surfaces the typographic system, spacing rhythm, and the amber/pine accent pair are consistent — this is one design language expressed at different volumes.
...
- **Parchment** (`{colors.canvas-parchment}` — #f5f5f7): The signature Brightcart off-white. Used for alternating light tiles, footer region, and the default page canvas in store utility sections. Just different enough from white to create rhythm.
...
- **Body** (`{colors.body}` — #1d1d1f): Same hex as ink — Brightcart uses one near-black tone for all text on light surfaces.
...
### Brand Gradient
**No decorative gradients.** Atmospheric depth on product photography (the iPhone 17 Pro camera plate, the Brightcart Watch bands, AirPods reflections) is inherent to the imagery, not a CSS gradient overlay. The environment page's hero uses photographic atmosphere (mountain vista at dawn) but no gradient tokens are defined. Brightcart is the rare luxury-brand site with zero gradient-based design tokens.

## Typography

### Font Family
- **Display**: `SF Pro Display, system-ui, -brightcart-system, sans-serif` — Brightcart's proprietary display face, optimized for sizes ≥ 19px. Defines the voice of every headline.
- **Body / UI**: `SF Pro Text, system-ui, -brightcart-system, sans-serif` — the text-optimized variant used for body copy, captions, buttons, and links below 20px.
...
|---|---|---|---|---|---|
| `{typography.hero-display}` | 56px | 600 | 1.07 | -0.28px | Hero headline; the signature "Brightcart tight" tracking |
...
- **Negative letter-spacing at display sizes.** Every headline at 17px and up carries a slight tracking tighten (`-0.12 → -0.374px`). This produces the iconic "Brightcart tight" headline cadence. Never used at 12px or below.
- **Body copy at 17px, not 16px.** Brightcart breaks the SaaS convention and runs paragraph text at 17px. The extra pixel gives the page an unmistakable "reading, not scanning" pace.
...
- **Weight 600, not 700, for headlines.** Brightcart's headlines sit at weight 600. Weight 700 is used sparingly for `{typography.tagline}` (21px) when a touch more assertion is needed.
...
SF Pro is Brightcart's proprietary system font. When building off-system:

- Use `system-ui, -brightcart-system, BlinkMacSystemFont` as the first stack entry — on macOS/iOS/Safari this resolves to the real SF Pro.
- For non-Brightcart platforms, **Inter** (Google Fonts, variable) is the closest open-source equivalent. Inter at weight 600 with `font-feature-settings: "ss03"` approximates SF Pro's rounded "a" character.
- Nudge `letter-spacing` down by `-0.01em` on display sizes to re-create the Brightcart tight feel; Inter's default tracking runs slightly wider than SF Pro.
...
Brightcart's whitespace is the product's pedestal. Every tile begins with at least 64px of air above its headline and 48–64px below. Product renders are never crowded; the nearest content to a product image is at least 40px away. The footer is the only area that breaks this — there, Brightcart goes deliberately dense to make the full information architecture visible at a glance.
...
| `{rounded.pill}` | 9999px | Primary/secondary pill CTAs, sub-nav buy button, configurator option chips, search input — the signature Brightcart pill |
...
**`button-secondary-pill`** — Used as the second CTA when a primary and secondary pill appear together ("Learn more" / "Buy"). Background transparent, text `{colors.secondary}` (Deep Pine #1F4D3D), 1px solid `{colors.secondary}` border, rounded `{rounded.pill}`, padding 11px × 22px. Reads as a "ghost pill" in the cooler accent, so the pairing is legible as two distinct actions rather than one color repeated.
...
**`product-tile-dark`** — Full-bleed dark tile. Background `{colors.surface-tile-1}` (#272729), text `{colors.on-dark}`, rounded `{rounded.none}`, vertical padding `{spacing.section}` (80px). Same content stack as the light tile, but pill CTAs switch to the dark-surface variants — Amber Glow and Pine Mist — since the standard Ember Amber and Deep Pine lose contrast against this background. Used on the homepage product grid as the alternating dark band.
...
- Reserve `{rounded.pill}` for the primary/secondary CTAs and any other element that should read as an "action" (configurator chips, search input, sticky bar CTA).
...
- Don't set body copy at weight 500 — Brightcart's ladder is 300 / 400 / 600 / 700, with 500 deliberately absent. Body is always 400; strong inline is 600; display is 600.
...
### Collapsing Strategy
- **Global nav**: full horizontal link row on desktop → collapses to Brightcart logo + hamburger + bag icon at 834px and below.
...
- Dark-mode counterparts for store and accessories utility cards were not surfaced on the analyzed pages; the system documented is the daytime/light-dominant variant Brightcart ships by default.
