...
|---|---|---|
| 1 | Login | Must |
| 2 | Forgot Password | Should |
| 3 | Set Password — invite acceptance | Must |

Identical to the agent tab. Listed once here for completeness; the only difference is where the user lands after login.

### 1 · Login

**Data points** — Brightcart logo · Email field (e.g. `manoj.p@brightcart.in`) · Password field with show/hide · "Remember me" · Sign in button · Forgot password link · Inline error ("Email or password is incorrect")

**Purpose** — Entry to the tool for all 16 users. Role determines landing screen: leads land on Team Overview, agents land on My Queue. There is no self-signup — accounts exist only by admin invite, because the user list is a fixed internal team of 16.
SOURCES: user record entered by admin; role entered by admin.

**Actions** — Sign in · Forgot password · Toggle password visibility

**Modals** — None

### 2 · Forgot Password

**Data points** — Email field · Send reset link button · Confirmation state ("If that email is registered, a reset link is on its way") · Back to login

**Purpose** — Self-service recovery so a locked-out lead does not have to wait on the other lead. Deliberately generic confirmation copy so the screen does not confirm which emails exist.
SOURCES: entered by person; email sent by platform.

**Actions** — Submit · Back to login

**Modals** — None

### 3 · Set Password — invite acceptance

**Data points** — "You've been invited to Brightcart Support Desk" · Invited name (e.g. Sneha Kulkarni) · Role being granted (e.g. Agent) · New password · Confirm password · Live rule checklist (8+ characters, one number) · Set password and continue

**Purpose** — Where a newly invited agent activates. Belongs in Manoj's tab because he is the one who sends the invite, and onboarding speed is one of his stated problems — "new agent joins, takes 3-4 weeks to understand our label system". Showing the role on this screen is deliberate: the new agent sees what access they were given rather than discovering it by trial.
SOURCES: invite record entered by admin; token generated by platform.

**Actions** — Set password · Request a new link if expired
...
**Data points** — Product name "Support Desk" · Primary nav: Team Overview · Unassigned (count badge, e.g. 9) · All Tickets · Reports · Settings · Global search ("Search tickets, customers, order IDs") · Notification bell with unread count · Profile menu (Manoj Patil · Support Lead · Settings · Sign out) · Last sync timestamp (e.g. "Synced 2 min ago")
...
**Actions** — Navigate · Global search · Open notifications · Open profile menu · Sign out

**Modals** — Sign out confirmation (only when a reply draft is unsaved)
